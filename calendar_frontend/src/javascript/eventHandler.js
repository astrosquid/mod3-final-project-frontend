...
function setSubmitListener() {
  ...
  submit.addEventListener('click', (event) => {
    ...
    .then( res => res.json() )
    .then( calDateJSON => {
      // console.log(calDateJSON)
      new CalendarDate(calDateJSON)
      ...
      eventAdapter.new({ title: title.value, location: location.value, calendar_date_id: calDateJSON.id})
      .then( res => res.json() )
      .then( eventJSON => {
        // console.log(eventJSON)
        new Event(eventJSON)
        // const newEventHTML = makeEventCards([eventJSON])
        // const container = document.getElementById('date-detail-container')
        // container.appendChild(newEventHTML[0])
        CalendarHandler.initCalendar()
        closeDetailView()
        const square = CalendarHandler.getSquareFromDate(date)
